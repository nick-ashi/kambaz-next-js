...
export const deleteAssignment = async (assignmentId: string) => {
  const { data } = await axiosWithCredentials.delete(`${COURSES_API}/assignments/${assignmentId}`);
  return data;
};
export const findAssignment = async (assignmentId: string) => {
  const { data } = await axiosWithCredentials.get(`${COURSES_API}/assignments/${assignmentId}`);
  return data;
};
